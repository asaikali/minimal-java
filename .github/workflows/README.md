...
| **`base-images.yml`** | changes under `images/2-size/{ubuntu,jre}/**` | Build + publish the base layer: `ubuntu`, then `jre` `FROM` the ubuntu **digest**. |
| **`artifact.yml`** | changes under `src/**`, `pom.xml`, … | Build the jar(s) and `oras push` them to `ghcr.io/<owner>/<repo>/jar` (plain + Spring-AOT). |
| **`containerize.yml`** | after base-images / artifact finish on `main` (`workflow_run`) | One matrix job per runtime image (`fat`, `app`, `jvm-aot`, `spring-aot`): `oras pull` the jar, build `FROM` the published `jre` digest, push. |
